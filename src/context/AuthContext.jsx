...
export default function AuthProvider({ children }) {
  ...
  async function login({ username, pin }) {
    ...
    if (storedData) {
      try {
        ...
        console.log('Merged user data on login:', {
          apiBalance: record.balance,
          storedBalance: parsedStored.balance,
          finalBalance: mergedUser.balance,
          transactionCount: mergedUser.transactions?.length || 0,
        });
        setUser(mergedUser);
        return mergedUser;
      } catch (e) {
  ...
  function logout() {
    if (user && user.id) {
      ...
      console.log('Saved user data before logout:', dataToSave);
    }
    setUser(null);
